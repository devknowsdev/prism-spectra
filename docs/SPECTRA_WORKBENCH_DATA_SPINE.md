...
- The workbench reads daemon-derived summaries for resume, conversations, attachments, approvals, changes, and capability metadata.
...
- Empty conversation, attachment, approval, and change states are intentional when the daemon has no corresponding records.
- Future project-memory cards, approval cards, provenance timelines, and status summaries should continue to read these manifest and daemon summaries instead of inventing new state.
...
- `GET /api/v1/workbench/conversations/:id`
- `GET /api/v1/workbench/attachments`
- `GET /api/v1/workbench/attachments/:id`
