...
export const seedCapabilityManifests: readonly CapabilityManifest[] = [
  {
    ...
    description: "Local-first attachment ingest and review scaffold for future Prism upload surfaces.",
    category: "attachment",
    source: {
      package: "@uppy/core",
    ...
    safety: {
      ...
      riskNotes: ["remote companions remain optional and disabled by default"],
    },
    io: {
      inputTypes: ["file", "attachment"],
      outputTypes: ["ingest-event", "preview-card"],
      sideEffects: ["reads_local_files", "prepares_ingest_preview"],
    },
    ui: {
      surfaces: ["attachments", "command_palette", "resume"],
      density: "calm",
    },
    provenance: {
    ...
    commands: {
      cli: ["prism capability run uppy.attachment.ingest"],
      api: ["POST /api/v1/capabilities/uppy.attachment.ingest"],
    },
    tests: {
      required: ["manifest-validation", "registry-rejection", "ingest-preview-contract"],
      fixtureTypes: ["attachment-list", "preview-card"],
    },
  },
  {
